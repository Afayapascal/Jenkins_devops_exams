...
stages {
        ...
        stage('Docker run'){ // run container from our builded image cast
                steps {
                    ...
                    docker run -d --name nginxjenkins1 $DOCKER_IMAGE_NGINX:$DOCKER_TAG_NGINX
		    sleep 6
		     docker run -d --name castpostgre_container1 -e postgres_user=cast_db_username -e postgres_password=cast_db_password -e postgres_db=cast_db_dev postgres:12.1-alpine
		    #sleep 10
		    ...
		    sleep 10
                     docker run -d -p 8001:8000 --name moviejenkins2 $DOCKER_ID/$DOCKER_IMAGE_MOVIE:$DOCKER_TAG_1
                    sleep 10
                     docker run -d -p 8002:8000 --name castjenkins2 $DOCKER_ID/$DOCKER_IMAGE_CAST:$DOCKER_TAG_1
                    '''
                    }
                }
            }
